...
evaluate.py
-----------
Evaluates the trained model on the test set, generates classification metrics, and saves performance plots (confusion matrix, ROC, PR curves) and reports.
"""
import joblib
import pandas as pd
...
def main():
    """
    Evaluate the trained model on the test set, print metrics, and save plots/reports.
    """
    try:
        ...
        logging.info("Classification report computed.")
        ...
        plt.close()
        logging.info("Precision-Recall curve plot saved.")

        # Save classification report to CSV
        report_df = pd.DataFrame(report).transpose()
        report_df.to_csv(f"{RESULTS_DIR}/final_classification_report.csv")
        logging.info("Classification report saved to CSV.")

        logging.info(f"✅ Evaluation complete. Plots saved to {PLOTS_DIR}, report saved to {RESULTS_DIR}")
